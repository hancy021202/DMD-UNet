...
from groundtruth import load_img
from dataset import data_Loader_all,data_Loader_one
...
model1_path = "Weights2_up100.pth"  # 存储路径
# model2_path = "Weights2_down100.pth"  # 存储路径
# model3_path = "Weights2_phase100.pth"  # 存储路径
################################################################
# ---------------------------warm up---------------------------#
################################################################

# ============= 3) Load Model + Loss + Optimizer + Learn_rate_update ==========#
transunet = TransUNet(img_dim=576,
                      in_channels=1,
                      out_channels=576,
                      head_num=4,
                      mlp_dim=512,
...
writer = SummaryWriter(logdir='./runs2/up100')  ## Tensorboard_show: case 2
...
def train(device, training_loader, validate_loader, start_epoch=0):
    ...
    for epoch in range(start_epoch, epochs, 1):
        ...
        if epoch % ckpt == 0:  # if each ckpt epochs, then start to save model
            torch.save(model1.state_dict(), 'Weights2_up100.pth')
            # torch.save(model2.state_dict(), 'Weights_down90.pth')
...
if __name__ == "__main__":
    ...
    #计算真值
    [x_up_train_label, x_down_train_label, x_phase_train_label, x_train_img] = load_img(train_path, [480, 480])
    [x_up_validate_label, x_down_validate_label, x_phase_validate_label, x_validate_img] = load_img(validate_path, [480, 480])
    #创建dataset
    train_dataset = data_Loader_one(x_train_img, x_up_train_label) #, x_down_train_label, x_phase_train_label)
    validate_dataset = data_Loader_one(x_validate_img, x_up_validate_label) #, x_down_validate_label, x_phase_validate_label)
    #载入数据集
