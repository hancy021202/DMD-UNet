...
def train(device, training_loader, validate_loader, start_epoch=0):
    ...
    for epoch in range(start_epoch, epochs, 1):
        ...
        if epoch % ckpt == 0:  # if each ckpt epochs, then start to save model
            ...
            # torch.save(model2.state_dict(), 'Weights2_down1.pth')
            # torch.save(model3.state_dict(), 'Weights2_phase90.pth')
...
if __name__ == "__main__":
    ...
    # 计算真值
    [x_up_train_label, x_down_train_label, x_phase_train_label, x_train_img_gray] = load_label(train_label_path,
                                                                                               [576, 576])
    [x_up_validate_label, x_down_validate_label, x_phase_validate_label, x_validate_img_gray] = load_label(
    ...
    validate_dataset = train_data_Loader(x_validate_img_rgb,
                                         x_down_validate_label)  # , x_down_validate_label, x_phase_validate_label)
    # 载入数据集
