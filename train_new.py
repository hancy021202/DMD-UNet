...
writer = SummaryWriter(logdir='./runs1/90')  ## Tensorboard_show: case 2
...
def train(device, training_loader, validate_loader, start_epoch=0):
    ...
    for epoch in range(start_epoch, epochs, 1):
        ...
        if epoch % ckpt == 0:  # if each ckpt epochs, then start to save model
            torch.save(model1.state_dict(), 'Weights_up90.pth')
...
if __name__ == "__main__":
    # 数据集载入
    train_path = 'D:\\活动\\大创\\工程文件\\解相unet\\data\\data_new\\train_new\\img_new\\'
    ...
    #创建dataset
    train_dataset = data_Loader(x_train_img, x_up_train_label, x_down_train_label, x_phase_train_label)
